...
const NoteSummary = ({title, text, _id, setEditRequiredNote}) => {
    const deleteNote = async e => {
        e.preventDefault();
        $($(e.target).closest('.card').get(0)).css({
            opacity: 0.5
        })
        try {
            await axios.delete(`https://boiling-basin-70629.herokuapp.com/api/notes/${_id}`);
            ...
            <div className='card'>
                ...
                <div className="button-sec">
                    <button onClick={deleteNote}>
                        <span className="icon delete-note">
                    ...
                    <button onClick={() => setEditRequiredNote(_id)}>
                        <span className="icon edit-note">
